    ...
    <body>
            ...
            <?php 
                if(!empty($_SESSION['logged']) && $_SESSION['logged'] == 1){
                    echo "<div class = 'dd-content' id = 'Acc'>
                            ...
                            <a href = 'Genres.php?genre=".$genre."&id=".$last_id."&logout=1'>Log out</a>
                        </div>
                    </div>";
                }
            ?>
                        ...
                        <div id = "GenresDD" class="dropdown-content">
                            <?php
                                foreach($category as &$value){
                                    if($genre == $value){
                                        echo "<a class = 'active genre' href = 'Genres.php?genre=".$value."&id=0'>".$value."</a>";
                                    }
                                    else{
        ...
        <div class = "content">
            ...
            <div class = "row">
                ...
                <?php
                    $sql = "SELECT Film_id, Title, Length, Rental_Rate FROM film WHERE Category = '".$genre."' AND Film_id > ".$id." Limit 40";
                    $result = $conn->query($sql);
                    if($result->num_rows > 0){
                        echo "<div class = 'Catheader'>";
                            echo "<h2>Movies in ".$genre." Category</h2>";
                        echo "</div>";
                        echo "<div class = 'Catbody'>";
                            while($row = $result->fetch_assoc()){
                                echo "<div class='card-view'>";
                                    echo "<div class='card-movie-content'>";
                                        echo "<div class='card-movie-content-head'>";
                                            echo "<a href='#'>";
                                                echo "<h3 class='card-movie-title'>".$row['Title']."</h3>";
                                            echo "</a>";
                                        echo "</div>";
                                ...
                                $mins = $row["Length"] - ($hours * 60);
                                                echo "<span>".$hours."hr".$mins."min</span>";
                                            echo "</div>";
                                            echo "<div class='movie-running-time'>";
                                                echo "<label>Price</label>";
                                                echo "<span class='movie-price'>".$row["Rental_Rate"]." MYR</span>";
                                            echo "</div>";
                                        echo "</div>";
                                    echo "</div>";
                                echo "</div>";
                            ...
                            echo "</div>";
                        $sql = "SELECT MAX(Film_id) AS maxid FROM film WHERE Category ='".$genre."'";
                        $result = $conn->query($sql);
                        if($result->num_rows > 0){
                            if(($row = $result->fetch_assoc())["maxid"] > $last_id){
                                echo "<div class = 'catfooter'>";
                                    echo "<a class = 'Morebtn' href = 'Genres.php?genre=".$genre."&id=".$last_id."'>More</a>";
                                echo "</div>";
                            }
                        }
                    }
