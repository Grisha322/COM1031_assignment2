            ...
            <?php 
                if(!empty($_SESSION['logged']) && $_SESSION['logged'] == 1){
                    echo "<div class = 'dd-content' id = 'Acc'>
                            ...
                            <a href = 'New.php?id=".$last_id."&logout=1'>Log out</a>
                        </div>
                    </div>";
                }
            ?>
        ...
        <header  id = "navbar">
            <nav>
                <ul>
                    ...
                    <li><a href="New.php?id=0" class = "active">New Arrivals</a></li>
                </ul>
            </nav>
        </header>
        <div class = "content">
            ...
            <div class = "row">
                ...
                <?php
                    $sql = "SELECT MAX(Release_year) AS Year FROM film";
                    $result = $conn->query($sql);
                    if($result->num_rows > 0){
                        ...
                        $sql = "SELECT Film_id, Title, Rental_Rate, Length FROM film WHERE (Release_Year = '".$latest_year."' AND Film_id > ".$_GET['id'].") LIMIT 80";
                    ...
                    $sql = "SELECT MAX(Film_id) AS maxid FROM film WHERE Release_year = ".$latest_year;
                    $result = $conn->query($sql);
                    if($result->num_rows > 0){
                        if(($row = $result->fetch_assoc())["maxid"] > $last_id){
                            echo "<div class = 'catfooter'>";
                                echo "<a class = 'Morebtn' href = 'New.php?id=".$last_id."'>More</a>";
                            echo "</div>";
                        }
                    }
